...
**Important:** The Nostr public key (64-char hex, matching `event.pubkey`) is **prepended** as the first element of the args array. All subsequent elements are the original args from the event content.

**Example Transformation:**

- **Input event content:** `{ "reducer": "player_move", "args": [origin, dest, run] }`
- **Output HTTP body:** `["32e1827635450ebb...", origin, dest, run]`
...
- Input pubkey: hex string (64 chars) from `event.pubkey`
- Output format: hex string (64 chars) — passed through as-is, no encoding

**Implementation:**

```javascript
function prepareReducerArgs(event, originalArgs) {
  // Prepend hex pubkey directly (no encoding needed)
  return [event.pubkey, ...originalArgs];
}

// Example:
// event.pubkey = "32e1827635450ebb3c5a7d12c1f8e7b2b514439ac10a67eef3d9fd9c5c68e245" (hex)
// originalArgs = [{ x: 100, z: 200 }, { x: 110, z: 200 }, false]
// returns: ["32e1827635450ebb...", { x: 100, z: 200 }, { x: 110, z: 200 }, false]
```
...
- [ ] Implement args array prepending logic (hex pubkey passthrough)
- [ ] Add unit tests:
  - Verify hex pubkey format is correct (64-char hex string)
...
- ✅ Hex pubkey passthrough is correct (64-char hex string)
...
["abc123def456...", {"x":100,"z":200}, {"x":110,"z":200}, false]
```
...
["xyz789uvw012...", {"itemId":42,"quantity":5}]
```
